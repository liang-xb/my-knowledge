...
<h1 style="text-align: center; font-weight: bold;">对数器</h1>
...
#### （1）你想要测的方法 a（<span style="color: red;">最优解</span>）

#### （2）实现复杂度不好但是容易实现的方法 b（<span style="color: red;">暴力解</span>）

#### （3）实现一个随机样本产生器（<span style="color: red;">长度也随机、值也随机</span>）
...
> #### 关键是第 5 步，找到一个<span style="color: red;">数据量小</span>的错误样本，便于你去带入 debug
>
> #### 然后把错误例子带入代码一步一步排查，Print 大法、断点技术都可以
>
> #### 对比两个方法每一步值的变化，找出值不一致的那一步，然后回到代码中找错
